...
## Section 4: Changing the Circle to Produce Different mmWaves

Welcome to chapter 4. So far we have understood that mmWaves are electromagnetic waves generated by an oscillating voltage which can be described using a circle. In this section, we will discuss more about the oscillating voltage and its relation with the electromagnetic waves so that we can understand this system more in depth.
...
This intensity difference is governed by the geometry of the oscillating voltage. If the oscillation takes place along the z-axis, then the electromagnetic waves released towards the x and y-axis would have maximum amplitude, and along the z-axis their amplitude will be zero. 

Everything else going from "parallel to oscillation" to "perpendicular to oscillation" will be in between zero and max amplitude. 
...
First, since the intensity is max in directions perpendicular to oscillation, and is zero in the direction of oscillation, if we plot the points out with the coordinates (I, $\theta$, $\phi$), where I is the intensity value, and $\theta$ and $\phi$ are the two angles at which the oscillation can rotate, we will get some kind of intensity mapping that looks like the donut formed in the end. This is called the radiation pattern.

Second, because the electromagnetic waves can be absorbed and reflected in air (due to small partcicles which exist in air), their amplitude decreases as they travel more distance. So in a sense, you can also picture this animation as a way to show in what direction the electromagnetic waves can travel further before their amplitude falls off.

We will come back to discuss the amplitude of the electromagnetic waves later. 

For now, let's first focus on the wavelength of the electromagnetic waves.

And since we've got ourselves familiarized with the circle from section 3, let's just use it to represent the oscillation above.
...
More specifically, let's look at the one that is perpendicular to oscillation and has maximum intensity. 
...
Note what is shown here on the right is the actual electromagnetic wave within one unit of wavelength. Not a sine function, which is one axis of the circle.
...
Mmmm... It seems like the wavelength of the electromagnetic waves is cut in half. Interesting...

What if we reduce the absolute voltage during oscillation? 
...
Oh... Now the cirlce has become smaller, and the amplitude of this wave has been decreased. 

This caused the strongest wave (along the perpendicular direction) to have an equivalent amplitude as those along some of the weaker directions (between parallel and perpendicular directions).

With this poor starting momentum, the electromagnetic wave will surely travel less distance before they turn into zero amplitude.

In fact, these waves will be no use to us well before they get to absolute zero amplitude -> as long as their amplitude is lower than most of the noisy signals that exist around them, they will offer no use to us because the Singal-to-Noise ratio is too low for us to tell them apart from noise.

Therefore, it is very important that we ensure a strong wave is produced in the first place.

## Section 5: How to Produce Stronger mmWaves
