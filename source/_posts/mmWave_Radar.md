...
We will now discuss more about the electromagnetic waves and their relation with the oscillating voltage. Our goal is to understand this system more in depth through this section. 
...
Everything else in between "parallel to oscillation" and "perpendicular to oscillation" will have some amplitude, but not maxed.

The viusal below might help to see this in 3D:
...
There are basically 2 ways to understand this animation. 
...
Second, because the electromagnetic waves can be absorbed and reflected when they travel (due to small partcicles that exist in air), their amplitude decreases as function of distance. So with that, you can also picture this animation as a way to show the directions in which the electromagnetic waves can travel further before their amplitude falls off.

We will come back to the amplitude of the electromagnetic waves later. 

For now, let's talk about their wavelength.

Also since we've got ourselves familiarized with the circle from section 3, let's just use it to represent the oscillation from above.
...
Now, let us zoom in to some of these waves generated by the circle, like we did for Figure 3.
...
Note what is shown here (on the right) is the actual electromagnetic wave within one unit of wavelength. It is not a sine function, which is one axis of the circle.

Now, have you ever wondered what is going to happen if we speed up the voltage oscillation? Let's see it with 2x the speed!
...
The lower voltage has caused the strongest wave (along the perpendicular direction) to have an equivalent amplitude such as those along some of the weaker directions (between parallel and perpendicular directions).

With the poor starting momentum, this wave will be less likely to travel as far before its amplitude vanishes.

In fact, these waves will be no use to us well before they get to absolute zero amplitude.

As long as their amplitude is lower than most of the noisy signals that exist around them, they will offer no value to us because it will be too hard for us to tell them apart from noise.

Therefore, it is very important that we guaranteed a strong wave is produced in the first place.
