...
In the case of a radar, the signals are generated by the voltage, but besides that we also send electromagnetic waves to space, to interact with things, get bounced back to the radar, and then turned into voltage signals again, so we can read. 

The returned signals in this case are shifted by a time difference $\tau$. That is how much time it took for the signal to get to an object and gets bounced back to the radar.

And the thing is, the sine and cosine functions have again thought about this, and made room for it! This can be expressed as Voltage(t) = $\sin(\omega(t-\tau))$.

Here, t is known (like, the 5th second in a cycle that repeats every 10s), w is known (it is just the frequency), and voltage can be measured. So you can solve for $\tau$. But there is a problem, this equation actually yields two results for $\tau$. 

How do you know which $\tau$ is right? You will need the second coordinate, x(t), which gives you x(t) = $\cos(\omega(t-\tau))$. If you solve this together with your Voltage(t) equation above, you will get four answers (two results from each equation), out of which, two answers are matching, meaning this $\tau$ satisfy both your y equation and your x equation, which is why, you need two coordinates (x, y). And two coordinates make up a circle. 
